...
\usepackage{subfig}
\usepackage{marvosym}
\usepackage[papersize={5.5in,8.5in},width=5in,height=7.25in]{geometry}
\usepackage{fancyhdr}
\pagestyle{fancy}
...
\draw[very thick] (0.1,.025)--(1.9,0.025);
\end{tikzpicture}
\item \begin{tikzpicture}[scale=.75]
\draw[->] (-1.9,0) -- (1,0);
\draw[<-] (-2.5,0) -- (-2.1,0);
...
\draw (-2,0)circle (.7ex);
\fill (0,0) node[below]{0} circle (.7ex);
\draw[very thick] (-1.9,0.025)--(-0.1,.025);
\end{tikzpicture}
\end{multicols}
\end{enumerate}
