...
\begin{enumerate}
 ...
 \begin{enumerate}
  \item -2,1,4,\ldots \addanswer*[0pt]{5 términos siguientes:}\answer*[0pt]{$a_{n}=$}
...
\begin{enumerate}
 \item $c_{n}=3n+3$ \answer*{Primeros diez términos}
 \item $d_{n}=3(2)^{n-1}$ \answer*{Primeros diez términos}
\end{enumerate}
...
\definecolor{cqcqcq}{rgb}{0.75,0.75,0.75}
\tikzpicture[scale=.75,line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
\draw [color=cqcqcq,dash pattern=on 2pt off 2pt, xstep=1.0cm,ystep=1.0cm] (-4.12,-5.44) grid (3.38,4.98);
\draw[->,color=black] (-4.12,0) -- (3.38,0);
\foreach \x in {-4,-3,-2,-1,1,2,3}
\draw[shift={(\x,0)},color=black] (0pt,2pt) -- (0pt,-2pt) node[below] {$\x$};
\draw[->,color=black] (0,-5.44) -- (0,4.98);
\foreach \y in {-5,-4,-3,-2,-1,1,2,3,4}
\draw[shift={(0,\y)},color=black] (2pt,0pt) -- (-2pt,0pt) node[left] {$\y$};
\draw[color=black] (0pt,-10pt) node[right] {$0$};
\clip(-4.12,-5.44) rectangle (7.38,4.98);
\draw[smooth,samples=100,domain=-3.0:0.99] plot(\x,{0-\x*\x+3});
\draw[smooth,samples=100,domain=1.0:4.0] plot(\x,{(\x)+2});
\draw [color=qqqqff] (1,3) circle (2pt);
\endtikzpicture
\end{center}
\begin{enumerate}
